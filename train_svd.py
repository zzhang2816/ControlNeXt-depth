...
from utils.dataset import WebVidCustom, WebVid10KDepth
...
def make_train_dataset(args):
    ...
    if args.dataset_type == 'webvid':
        ...
        dataset = WebVidCustom(
            video_folder="/data2/wff/webvid",
            depth_folder="/data2/zhiyuan/depth_frames",
            sample_size=(288, 512),
            sample_n_frames=14
        )
    elif args.dataset_type == 'webvid10k':
        dataset = WebVid10KDepth(args.csv_path,args.video_folder,args.condition_folder,args.motion_folder)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        # val_vids = os.listdir(args.validation_image_folder) 
                        # val_vids = random.sample(val_vids, 3)
                        val_vids = ["15559555"]
                        for vid in val_vids:
                            validation_images = load_images_from_folder(os.path.join(args.validation_image_folder, f"{vid}/ground_truth"))
                            validation_image = validation_images[0]
                            validation_control_images = load_images_from_folder(os.path.join(args.validation_image_folder, f"{vid}/depth"))
                            
                            # run inference
                            val_save_dir = os.path.join(
                                args.output_dir, f"validation_images/{global_step}")
